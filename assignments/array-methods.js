...
console.log(`Donation total is $${ticketPriceTotal}`);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 Use .forEach()
//There was an unexpected change to the itenerary and the event director needs the full names and email addresses to send updates immediately.

let runnerEmails = [];
runners.forEach(function(item) {
    return runnerEmails.push(`${item.first_name} ${item.last_name}: ${item.email}`);
});

console.log(runnerEmails);

// Problem 2 Use .filter()
// Due to unforseen circumstances, all companies must donate atleast $25 to participate. Return a list of companies with donations less than $25.

let companyList = [];
runners.filter(function(item) {
    if (item.donation < 25) {
        companyList.push(`${item.company_name} donated ${item.donation}.`);
        return companyList;
    }
});
...
// Problem 3
//The Power trip continues! the event director hates looking at the names in all caps and thinks lowercase is the way to go. Produce a new list with first and last names in lowercase.

let lowercasedNames = runners.map(function(element) {
    return element.first_name.toLowerCase();
});
